  ...
  for (JsonObject::iterator it = fileseq.begin(); it!=fileseq.end(); ++it)
  {
    ...
    if ( SD.exists( thefile ) )
    {
      ...
      */
      ...
      {
        // Download the file
        if ( ! getFileSaveToSD( arcfile ) ) return;
...
{
  Serial.println( "removeall, removing files" );

  if ( removeFiles( SD, "/", 100 ) )
  {
    Serial.println("Removing directories");
    
    File root = SD.open( "/" );
    if(!root){
        Serial.println("removeall - Failed to open / directory");
        return false;
    }
  ...
  {
    Serial.println( "Storage::removeall returned error" );
  }
  return true;
}
...
void Storage::rm( File dir, String tempPath )
{
  while(true) 
  {
    ...
    if ( ! entry )
    {
      Serial.print( "rm failed on dir.openNextFile for ");
      Serial.print( dir.path() );
      return;
    }
