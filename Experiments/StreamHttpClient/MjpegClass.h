...
{
public:
  bool setup(File input, uint8_t *mjpeg_buf, Arduino_TFT *tft, bool multiTask, bool firsttime)
  {
    _input = input;
    ...
    if ( firsttime )
    {
      ...
      for (int i = 0; i < 2; ++i)
      {
        // not sure why this is * 48 (below)? -Frank
  
        _out_bufs[i] = (uint8_t *)heap_caps_malloc(_tft_width * 48 * 2, MALLOC_CAP_DMA);
  
        if ( _out_bufs[i] == NULL )
        {
          Serial.print( "Error on allocating read buffer " );
          Serial.print( i );
          Serial.print( " with size " );
          Serial.println( _tft_width * 48 * 2 );
  
        }
        else
        {
          Serial.print( "Allocated read buffer " );
          Serial.print( i );
          Serial.print( " with size " );
          Serial.println( _tft_width * 48 * 2 );
        }
    ...
    _file_size = input.size();  // Get file size
    _file_count = _file_size;
  ...
  /*
   * Read the next image into the buffer
   */
    ...
    if (_buf_read > 0)
    {
      ...
      while ( (_buf_read > 0) && (!found_FFD9) && ( _file_count > 0 ) )
      {
        ...
        */
        ...
        // Serial.print( " _inputindex=" );
        // Serial.println( _inputindex );
...
private:
  ...
  int32_t _file_size = 0;
  int32_t _file_count = 0;
  ...
  /*
   * Hidden in here is the _out_bufs being flipped, this is double buffering
   * the video stream. Nice! -Frank
   */
