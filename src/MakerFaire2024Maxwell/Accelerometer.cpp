...
{ 
  ...
  lastShakeTime = millis(); // For Maker Faire 2024 demo
  franktime = millis();
}
...
void Accelerometer::loop()
{
  ...
  if ( millis() - lastShakeTime > shakeDuration )
  {
    ...
    */
    ...
    if (totalAccel > shakeThreshold) 
    {
      ...
      {
        timeservice.startShow( 2 );   // Shows bird reaction to mysitc message
        timeservice.setTimeAnimationActivated( true );

        // Make RPC call to Maxwell bird

        // const char* serverURL = "http://10.0.0.180:59224/CanAnimate/";
        const char* serverURL = "http://192.168.8.184:59224/PlaybackState/";

        HTTPClient http;
        http.begin(serverURL);        
        http.addHeader("Content-Type", "application/json");
        String jsonPayload = "{\"selectedAnimationIndex\":";
        jsonPayload += timeservice.getMysticType();
        jsonPayload += " }";
        ...
        const char* serverURL2 = "http://192.168.8.184:59224/PlaybackState/";

        HTTPClient http2;
        http2.begin(serverURL);        
        http2.addHeader("Content-Type", "application/json");
        String jsonPayload2 = "{\"isPlaying\":1 }";

        Serial.print( "isPlaying:  " );
        Serial.println( jsonPayload2 );

        int httpResponseCode2 = http2.PUT(jsonPayload2);
        ...
        {
          Serial.printf("Error in HTTP request: %s\n", http2.errorToString(httpResponseCode2).c_str());
        }
