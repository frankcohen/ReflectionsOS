...
**Maker Faire 2024 Technology Demonstration**

Frank Cohen exhbited the Reflections board and technology at the Maker Faire Bay Area 2024.
A foamcore board has a Reflections board, battery, and display to show a fortune telling 
entertaining experience. Shake the board and a hopeful, benign, and negative answer 
appears on the display. The flamboyant parrot knods in agreement for hopeful answers, 
nods left and right for benign ansers, and bows his head for negative answers. 

Reflections board is an Arduino IDE 2 compatible ESP32-S3 with 240 x 240 pixel TFT
display. Makes remote procedure calls to a Bottango Maxwell animatronic over Wifi.
Maxwell driven by a MacOS laptop running Bottango software. Reflections sketch
software at:
https://github.com/frankcohen/ReflectionsOS/src/MakerFaire2024Maxwell

Maxwell animatronic figure and software at
https://www.bottango.com/products/maxwell-1

Plays animation 0:
curl -X PUT -H "Content-Type: application/json" -d '{"isPlaying": 1}' http://localhost:59224/PlaybackState/
Bottango app needs to be in Animate mode, with </> API turned on in Hardware mode
...
void setup() {
  ...
  Serial.println("- - -");
  Serial.println(F("Maker Faire 2024 Reflections Demo"));
  Serial.println(" ");
...
void nothinghere()
{
  ...
  if ( ( millis() - maxIdleTimer ) > 30000 )
  {
    maxIdleTimer = millis();

    if ( WiFi.status() != WL_CONNECTED ) return;

    const char* serverURL = "http://10.0.0.180:59224/PlaybackState/";
    ...
    if (httpResponseCode > 199) 
    {
      ...
      DynamicJsonDocument doc(2048); // Increase size for larger JSON responses
      DeserializationError error = deserializeJson(doc, response); // Deserialize the JSON
    ...
    if ( ! isPlaying )
    {
      ...
      */
      ...
      String jsonPayload = "{\"isPlaying\": 4}";      
